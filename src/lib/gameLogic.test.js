...
describe("monsterImage", () => {
  it("returns boss image on stage 5", () => {
...
describe("applyTap", () => {
  ...
  it("advances stage on killing blow and resets hp to new stageHp", () => {
    const state = { ...createInitialState(), hp: 1 };
    const next = applyTap(state);

    expect(next.stage).toBe(2);
    expect(next.hp).toBe(stageHp(2));
    expect(next.maxHp).toBe(stageHp(2));
    expect(next.gold).toBe(1);
